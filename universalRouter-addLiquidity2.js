...
const domainB = {
    ...
    version: "1",
    chainId: 11155111,
    verifyingContract: WBTC_ADDRESS,
}
...
async function main() {
    ...
    const userPermitNonceA = await gt3Contract.read.nonces([account.address])
    const userPermitNonceB = await wbtcContract.read.nonces([account.address])

    const messageA = {
        owner: account.address,
        spender: ROUTER_ADDRESS,
        value: parseEther("1"),
        nonce: userPermitNonceA,
    ...
    const signatureA = await walletClient.signTypedData({
        account,
        domain: domainA,
        types,
        primaryType: "Permit",
    ...
    const parsedSignatureA = parseSignature(signatureA)

    //hago esto para ahorrar tiempo, hacedlo como lo tengáis
    const quote = await routerContract.read.quoteAddLiquidity([GT3_ADDRESS, WBTC_ADDRESS, messageA.value, maxUint256])

    const messageB = {
        owner: account.address,
        spender: ROUTER_ADDRESS,
        value: quote[1],
        nonce: userPermitNonceB,
        deadline: 115792089237316195423570985008687907853269984665640564039457584007913129639935n //usar parseInt(Date.now() / 1000) + 600 p.ej
    }

    const signatureB = await walletClient.signTypedData({
        account,
        domain: domainB,
        types,
        primaryType: "Permit",
    ...
    const parsedSignatureB = parseSignature(signatureB)
    ...
    await walletClient.writeContract({
        ...
        args: [
            [GT3_ADDRESS, WBTC_ADDRESS],
            [messageA.value, messageB.value],
            [0n, 0n], //esto no será 0, pero ya lo tendréis calculado, eso sigue igual
            account.address,
            messageA.deadline,
            [
                { v: parsedSignatureA.v, r: parsedSignatureA.r, s: parsedSignatureA.s },
                { v: parsedSignatureB.v, r: parsedSignatureB.r, s: parsedSignatureB.s }
            ]
        ],
        account
    })
